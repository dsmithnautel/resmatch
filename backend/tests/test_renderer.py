...
async def test_render_resume_success():
    ...
    selected_units = [
        ScoredUnit(unit_id="u1", text="text", section="experience", org="o1", role="r1", llm_score=9.0)
    ]
    ...
    mock_cursor.__aiter__.return_value = [
        {"text": "John Doe", "type": "header"}
    ]
    mock_db.atomic_units.find.return_value = mock_cursor

    mock_cv_data = {"cv": {"name": "John Doe"}}
    
    with patch("app.services.renderer.get_database", return_value=mock_db), \
